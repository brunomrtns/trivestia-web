...
export class SimulationEngine {
  ...
  placeOrder(order, candle, candleIndex) {
    ...
    if (order.quantity > this.config.maxPositionSize) return;
    ...
    if (order.type === 'MARKET') {
      ...
      const fillPrice =
        order.side === 'BUY' ? rawPrice + slippageAmt : rawPrice - slippageAmt;
      this.fillOrder(
        order,
        fillPrice,
        rawPrice,
      ...
      if (this.state.openOrders.length >= this.config.maxOpenOrders) return;
  ...
  fillOrder(
    ...
    const feeValue =
      (fillPrice * order.quantity * this.config.feeBps) / 10000 +
  ...
  updatePosition(fill) {
    ...
    if (pos.side === 'FLAT') {
      ...
      pos.entryPrice = round2(
        (pos.entryPrice * pos.quantity + fill.fillPrice * fill.quantity) /
          totalQty
      );
  ...
  checkSlTp(candle, candleIndex) {
    ...
    this.activeSlTps = this.activeSlTps.filter(
      (s) => !toRemove.includes(s.orderId)
    );
  }
  ...
  updateEquity(candle) {
    ...
    if (pos.side === 'FLAT') {
      ...
      const currentPrice =
        pos.side === 'LONG' ? candle.bidClose : candle.askClose;
      pos.unrealizedPnl =
        pos.side === 'LONG'
