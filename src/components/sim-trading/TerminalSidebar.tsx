...
const tools = [
  ...
  { id: 'speed', icon: Zap, labelKey: 'sim.terminal.tools.speed' },
  { id: 'indicators', icon: LineChart, labelKey: 'sim.terminal.tools.indicators' },
  ...
  { id: 'settings', icon: Settings2, labelKey: 'sim.terminal.tools.settings' },
];

export function TerminalSidebar({ activeTool, onToolChange }: TerminalSidebarProps) {
    ...
    <aside className="w-[60px] border-r border-border bg-card flex flex-col items-center py-4 gap-4 shrink-0 z-20 shadow-xl">
      ...
      <div className="flex flex-col gap-2.5 w-full px-2">
        {tools.map((tool) => (
          ...
          >
            <tool.icon className={cn("h-5 w-5 transition-transform duration-300 group-hover:scale-110", activeTool === tool.id ? "scale-100" : "")} />
