...
type PriceScaleRangeSubscriptions = {
  subscribePriceRangeChange?: (handler: () => void) => void;
  unsubscribePriceRangeChange?: (handler: () => void) => void;
};
...
export function DrawingOverlay({
  ...
  useLayoutEffect(() => {
    ...
    const priceScaleApi = chart.priceScale(
      'right'
    ) as PriceScaleRangeSubscriptions;
          ...
          <div className="flex items-center gap-3 rounded-lg border border-primary/30 bg-background/95 px-4 py-2.5 shadow-lg backdrop-blur-md">
            ...
            <div
              className={`h-3 w-3 rounded-full flex-shrink-0 ${
                currentLine ? 'bg-primary animate-pulse' : 'bg-muted-foreground'
              }`}
            />
