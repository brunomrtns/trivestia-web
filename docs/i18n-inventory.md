...
| **Lote 6**       | AnnouncementBell.tsx · AnnouncementFormModal.tsx · PeriodFormModal.tsx · CourseInteractiveHeader.tsx · CourseOutlineSidebar.tsx · LessonLockBadge.tsx                                                                                              | ✅ Concluído |
| **Lote 7**       | StepPlayer.tsx · MultipleSelectRenderer.tsx · OrderingRenderer.tsx · TextInputRenderer.tsx · ChartMarkupRenderer.tsx · RiskCalculatorRenderer.tsx · LessonTimeline.tsx · CourseInlineLessonPlayer.tsx                                              | ✅ Concluído |
| **Lote 8**       | AdminLessonStepsPage.tsx · StepFormModal.tsx · QuestionPreviewCard.tsx · SuperDashboardPage.tsx · SuperTenantsPage.tsx · SuperTenantEditPage.tsx · SuperUsersPage.tsx                                                                              | ✅ Concluído |
| **Lote 9**       | ChartMarkupQuestionForm.tsx · RiskCalculatorQuestionForm.tsx · SimTradingQuestionForm.tsx · SuperSidebar.tsx · SuperTopbar.tsx                                                                                                                     | ✅ Concluído |
...
## Lote 3 — Student pages + Platform pages + Dashboard components ✅ MIGRADO
...
## Lote 4 — Admin Pages + Learning Components (10 arquivos) ✅ MIGRADO
...
- `src/components/learning/MultipleChoiceRenderer.tsx` _(sem strings — letras computadas via `String.fromCharCode`)_
...
- `super.layout` — 7 novas chaves (brand, roleBadge, nav._, aria._)
...
## Limpeza Final — Varredura completa do codebase ✅ MIGRADO

> Data: **08/03/2026**. Grep varredura de todas as strings PT-BR restantes. Zero strings hardcoded após esta sessão.

**Arquivos cobertos:**

- `src/layouts/TenantAuthLayout.tsx` — brand name fallback `'Trivestia'`
- `src/layouts/TenantPublicLayout.tsx` — brand name fallback `'Trivestia'` (header + footer)
- `src/components/admin/ChartMarkupQuestionForm.tsx` — `alt="Preview"`
- `src/pages/admin/AdminQuestionsPage.tsx` — `alt="Preview"`
- `src/routes/guards/AdminGuard.tsx` — `toast.error` PT-BR
- `src/routes/guards/SuperAdminGuard.tsx` — `toast.error` PT-BR
- `src/components/learning/ActivityPlayerContent.tsx` — 11 strings (toasts, botões, textos de resultado)
- `src/components/learning/StepPlayer.tsx` — `title="Video player"` no iframe
- `src/components/sim-trading/useSimEngine.ts` — 2 fallbacks de erro de API
- `src/pages/admin/AdminLessonsPage.tsx` — `toast.info` de materialização
- `src/pages/admin/AdminLessonsPage.tsx`, `AdminQuestionsPage.tsx`, `src/pages/student/ActivityPlayerPage.tsx`, `ProgressPage.tsx`, `DashboardPage.tsx`, `src/components/learning/ActivityPlayerContent.tsx` — remoção de `getActivityTypeLabel()` / `getProgressLabel()`, substituídos por `t('common.activityTypes.*')` / `t('common.progressStatus.*')`
- `src/lib/utils.ts` — funções `getProgressLabel` e `getActivityTypeLabel` **removidas** (dead code após migração)

| #   | Categoria | Arquivo                    | Texto PT-BR                                                                         | Chave                                           | Notas                                      |
| --- | --------- | -------------------------- | ----------------------------------------------------------------------------------- | ----------------------------------------------- | ------------------------------------------ |
| 867 | ui        | TenantAuthLayout.tsx       | Trivestia                                                                           | `common.brandName`                              | Fallback brand name × 2 (span)             |
| 868 | ui        | TenantPublicLayout.tsx     | Trivestia                                                                           | `common.brandName`                              | Fallback no header + footer © × 2          |
| 869 | aria      | ChartMarkupQuestionForm.tsx | Prévia da imagem                                                                   | `admin.questions.form.previewAlt`               | alt do img de preview da imagem do enunciado |
| 870 | aria      | AdminQuestionsPage.tsx     | Prévia da imagem                                                                    | `admin.questions.form.previewAlt`               | Reusa #869                                 |
| 871 | toast     | AdminGuard.tsx             | Acesso restrito a administradores.                                                  | `common.guards.adminOnly`                       | Era hardcoded; chave já existia no JSON    |
| 872 | toast     | SuperAdminGuard.tsx        | Acesso restrito ao Super Admin da plataforma.                                       | `common.guards.superAdminOnly`                  | Era hardcoded; chave já existia no JSON    |
| 873 | toast     | ActivityPlayerContent.tsx  | Atividade concluída!                                                                | `learning.activityPlayer.toast.success`         | toast.success ao submeter                  |
| 874 | toast     | ActivityPlayerContent.tsx  | Erro ao enviar respostas. Tente novamente.                                          | `learning.activityPlayer.toast.error`           | toast.error                                |
| 875 | empty     | ActivityPlayerContent.tsx  | Nenhuma questão cadastrada                                                          | `learning.activityPlayer.noQuestions`           | Estado vazio — sem questões                |
| 876 | empty     | ActivityPlayerContent.tsx  | Esta atividade ainda não possui questões. Peça ao administrador para adicioná-las. | `learning.activityPlayer.noQuestionsHint`       |                                            |
| 877 | ui        | ActivityPlayerContent.tsx  | Parabéns! Você passou.                                                              | `learning.activityPlayer.passed`                | Resultado aprovado                         |
| 878 | ui        | ActivityPlayerContent.tsx  | Continue praticando!                                                                | `learning.activityPlayer.failed`                | Resultado reprovado                        |
| 879 | ui        | ActivityPlayerContent.tsx  | Revisão das questões                                                                | `learning.activityPlayer.reviewTitle`           | h2 da seção de revisão                     |
| 880 | ui        | ActivityPlayerContent.tsx  | Questão {{n}}                                                                       | `learning.activityPlayer.questionLabel`         | Cabeçalho de cada questão                  |
| 881 | ui        | ActivityPlayerContent.tsx  | Tentar novamente                                                                    | `learning.activityPlayer.retryButton`           | Botão reiniciar                            |
| 882 | toast     | ActivityPlayerContent.tsx  | Responda todas as questões antes de enviar.                                         | `learning.activityPlayer.answerAllWarning`      | toast.warning                              |
| 883 | ui        | ActivityPlayerContent.tsx  | Anterior                                                                            | `learning.activityPlayer.prevButton`            | Botão nav anterior                         |
| 884 | ui        | ActivityPlayerContent.tsx  | Próxima                                                                             | `learning.activityPlayer.nextButton`            | Botão nav próxima                          |
| 885 | ui        | ActivityPlayerContent.tsx  | Enviar atividade                                                                    | `learning.activityPlayer.submitButton`          | Botão submit final                         |
| 886 | aria      | StepPlayer.tsx             | Video player                                                                        | `learning.stepPlayer.videoTitle`                | title do iframe — chave já existia no JSON |
| 887 | error     | useSimEngine.ts            | Erro ao carregar cenário                                                            | `sim.engine.loadError`                          | Fallback quando API não retorna msg        |
| 888 | error     | useSimEngine.ts            | Erro ao enviar resultado                                                            | `sim.engine.submitError`                        | Fallback quando API não retorna msg        |
| 889 | toast     | AdminLessonsPage.tsx       | Aguardando materialização das etapas...                                             | `admin.lessonSteps.toast.materializationPending`| toast.info ao reordenar com etapas virtuais|
| 890 | ui        | Vários (6 arquivos)        | Múltipla Escolha / Seleção / V ou F / Ordenação / Resp. Aberta / Cenário / …       | `common.activityTypes.*` (9 entradas)           | Substituiu `getActivityTypeLabel()` inline |
| 891 | ui        | Vários (2 arquivos)        | Concluído / Em andamento / Não iniciado                                             | `common.progressStatus.*` (3 entradas)          | Substituiu `getProgressLabel()` inline     |

### Novas chaves adicionadas ao pt-BR.json

- `common.brandName` — `"Trivestia"` — nome da plataforma como fallback
- `common.activityTypes` — 9 entradas (MULTIPLE_CHOICE → SIM_TRADING_CHALLENGE) — substitui `getActivityTypeLabel()` em utils.ts
- `common.progressStatus` — 3 entradas (COMPLETED, IN_PROGRESS, NOT_STARTED) — substitui `getProgressLabel()` em utils.ts
- `admin.questions.form.previewAlt` — `"Prévia da imagem"`
- `admin.lessonSteps.toast.materializationPending` — `"Aguardando materialização das etapas..."`
- `learning.activityPlayer` — 12 entradas (toast.success/error, noQuestions, noQuestionsHint, passed, failed, reviewTitle, questionLabel, retryButton, answerAllWarning, prevButton, nextButton, submitButton)
- `sim.engine` — 2 entradas (loadError, submitError)
- **Total Limpeza Final: 30 novas chaves** | Arquivos modificados: 14 | Funções removidas: `getProgressLabel`, `getActivityTypeLabel` de `lib/utils.ts`
